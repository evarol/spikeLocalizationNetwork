...
## Drift-correction comparison — DREDge vs MEDICINE

The localizer (MP or SLN) is orthogonal to the *drift estimator*. We support two
motion-correction back-ends and compare them head-to-head:

- **DREDge** — pairwise histogram cross-correlation (via `dredge_wrapper.py`).
- **MEDICINE** ([`medicine-neuro`](https://jazlab.github.io/medicine/)) — a deep
  contrastive method that jointly infers probe motion and the activity
  distribution. `src/run_medicine_correction.py` is a drop-in wrapper: it takes an
  uncorrected localization `GL_pre` (+ spike times and amplitudes), runs
  `medicine.run_medicine` on the depth axis, and writes drift-corrected `x/y/z`
  in the same layout as the DREDge path.

```bash
python3 src/run_medicine_correction.py \
    --gl_pre <apply_dir>/GL_pre_dredge.npy \
    --spike_times results/spike_times.npy \
    --amplitudes results/spike_amplitudes.npy \
    --fs results/fs.npy \
    --out_dir results/mp_medicine --num_depth_bins 5
```

Three evaluation views compare **raw / +DREDge / +MEDICINE** across the MP and SLN
localizers (2×3 grids, generated by `src/render_ncc_matrices.py` and
`src/render_depth_raster_grid.py`):

- **Pairwise-NCC matrices** (`figures/comparison/ncc_matrix_grid_np2.png`) — the
  T×T correlation of drift-corrected 1-s localization histograms.
- **Depth-vs-time rasters** (`figures/comparison/depth_raster_grid_np2.png`) — the
  amplitude-weighted activity bands; drift makes them wave, a good correction
  straightens them.

![drift raster comparison](figures/comparison/depth_raster_grid_np2.png)

And a **comparison interactive visualizer** (`src/make_comparison_interactive_html.py`)
— one self-contained HTML per dataset with a method toggle (raw / DREDge / MEDICINE
× MP / SLN) that live-swaps the aggregate localization cloud, each labeled with its
pairwise-NCC ρ̄:

![comparison interactive](figures/comparison/comparison_interactive.png)

```bash
python3 src/make_comparison_interactive_html.py --dataset both
# writes figures/comparison_interactive_{np2,npu}.html
```

On our data MEDICINE and DREDge are near-interchangeable (e.g. MP+MEDICINE ρ̄=0.59
vs MP+DREDge 0.58; SLN+MEDICINE 0.64 vs SLN+DREDge 0.66), with the SLN localizer
giving the tightest clouds under either corrector.
